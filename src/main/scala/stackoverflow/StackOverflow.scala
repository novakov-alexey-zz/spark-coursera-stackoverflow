...
class StackOverflow extends Serializable {
    ...
    grouped.flatMap {
      case (_, list) =>
        list.groupBy(_._1).map {
          case (question, pairs) => question -> answerHighScore(pairs.map(_._2).toArray)
        }
    }
  }
  ...
  def vectorPostings(scored: RDD[(Posting, Int)]): RDD[(Int, Int)] = {
    ...
    scored
      .map { case (p, hScore) => firstLangInTag(p.tags, langs) -> hScore }
      .filter(_._1.isDefined)
      .map { case (Some(i), hScore) => i * langSpread -> hScore }
      .cache()
  }
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    val newMeansValues = closest.groupByKey().mapValues(vector => averageVectors(vector)).collect().toMap
    val newMeans = means.indices.map(i => newMeansValues.getOrElse(i, means(i))).toArray
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val langPercent: Double = 100 * vs.count(_._1 == lang) / vs.size // percent of the questions in the most common language
